...
namespace I_RAY_ASSET_TRACKER_MVC.Controllers
{
    public class AssetClearanceLetterController : Controller
    {
        ...
        [AllowAnonymous]
        [HttpPost]
        public ActionResult Index(AssetClearanceLetterModel assetClearanceLetterModel)
        {
            string EmployeeName = null;
            SqlConnection con = new SqlConnection(constr);
            if (ModelState.IsValid)
            {
                SqlCommand cmd = new SqlCommand("select count(*) from AssignAsset where EmpID=@EmpID");
                cmd.CommandType = CommandType.Text;
                cmd.Connection = con;
                con.Open();
                cmd.Parameters.AddWithValue("@EmpID", assetClearanceLetterModel.EmpID);
                ...
                if (i == 0)
                {
                    ...
                    using (System.IO.MemoryStream memoryStream = new System.IO.MemoryStream())
                    {
                        ...
                        Response.Flush();
                        Response.End();
                        //TempData["SuccessMessage"] = "PDF download successful!";
                    }
                }
                else
                {
                    TempData["ErrorMessage"] = "Still employee seems to return his assets.please return them to asset manager.";
                }
            }
